"""Точка входа для загрузки и извлечения тендерной документации.

Использование:
    python ingest.py <tender_id>
    python ingest.py <tender_id> --force
    python ingest.py 12345678 87654321    # несколько тендеров
"""
...
from tender_tools import TenderPipeline
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Скачать и извлечь документацию тендера по ID"
    )
    parser.add_argument(
        "tender_ids",
        nargs="+",
    ...
    parser.add_argument(
        "--force",
        action="store_true",
        help="Принудительно перескачать и переизвлечь",
    )
    parser.add_argument(
        "-w", "--workers",
        type=int,
